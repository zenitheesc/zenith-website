...
function Zenith() {
  ...
  return (
    <>
      ...
      />
      <NavBar />
      <HeroSection backgroundImage="url(../images/Zenith/Header.webp)" title={title} subtitle={subtitle} array={array} />
      <CardContainer>
        <Card
        ...
        <Card
          ...
          right={
            <>
              ...
              {card3.text.map((c, i) => (
                <TextWithIcons content={c} key={i} index={i + 1} icons={icons} />
              ))}
              <br />
            </>
          }
        ...
        <Card
          ...
          left={
            <>
              ...
              {card4.text.map((a, idx) => (
                <p key={idx}>{boldParser(a)}</p>
              ))}
              {card4.links.map((l, idx) => (
                <UnderlinedButton key={idx} link={l.url} label={l.text} dark />
              ))}
            </>
          }
          text={'left'}
