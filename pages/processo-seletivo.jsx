...
function mapTable(content, idx) {
  return (
    <p className={styles.cronogram} key={idx}>
      <b>{content[0]}</b>: <span key={idx}>{content[1]}</span>
    </p>
  );
}
        ...
        <Card
          ...
          right={
            <>
              ...
              {TextContent.processoSeletivoCard1.text.map((a, idx) => (
        ...
        <Card
          ...
          right={
            <>
              ...
              {TextContent.processoSeletivoCard3.text.map((a, idx) => (
                <p key={idx}>{boldParser(a)}</p>
              ))}
            </>
          }
          text={'right'}
        />
        {Config.processoSeletivoOpen ? <FormWrapper className={'-section'} link={Config.processoSeletivoForms} /> : <> </>}
      </CardContainer>
      <Footer />
    </>
  );
